...
function run_experiment_for_defense {
	...
	ip netns exec client-net tcpdump -i veth0 -w ${BASE_DIR}${DEFENSE}/${msmID}-${shortname}/client.pcap 2> /tmp/tcpdump-client.log  &
	tcpdumpclientPID=$!
	ip netns exec bottleneck-net tcpdump -i br-client-inet -w ${BASE_DIR}${DEFENSE}/${msmID}-${shortname}/middle.pcap 2> /tmp/tcpdump-middle.log &
	tcpdumpmiddlePID=$!
	#tcpdumpserverPIDS=()
